...
int check_feasibility_diff_rhs(int level, MIPdesc *mip,
				 int rhs_cnt, int *new_rhs_ind, double *new_rhs_val,
				 int lb_cnt, int *new_lb_ind, double *new_lb_val,
				 int ub_cnt, int *new_ub_ind, double *new_ub_val,
				 double *objval)
{
   ...
   int lbubcnt = mip->n;
   int *lbubind = (int *) malloc(ISIZE * lbubcnt);
   double *lbval = (double *) calloc(DSIZE, lbubcnt);
   double *ubval = (double *) calloc(DSIZE, lbubcnt);
   for (i = 0; i < lbubcnt; i++) {
      lbubind[i] = i;
   }
   for (i = 0; i < lb_cnt; i++) {
      lbval[new_lb_ind[i]] = new_lb_val[i];
   }
   for (i = 0; i < ub_cnt; i++) {
      ubval[new_ub_ind[i]] = new_ub_val[i];
   }
   for (i = 0; i < lbubcnt; i++){
